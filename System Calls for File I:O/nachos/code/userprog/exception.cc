...
ExceptionHandler(ExceptionType which)
{
  ...
  switch (which) {
    case SyscallException:
	  ...
	  int retVal = read(id, buffer, size);
	  if(retVal < 0){
	  ...
	  for(int i = 0; i < size; i++){
	  	kernel->machine->WriteMem(temp+i, 1, (int)buffer[i]);
	  }
	  delete(buffer);
	  break;
	  }
	  ...
	  int id = kernel->machine->ReadRegister(6);
          ...
          int i = 0;
          while( i < 99 && chr != 0){
                  kernel->machine->ReadMem(temp+i, 1, &chr);
		  buffer[i++] = char(chr);
          }
 	  int retVal = write(id, buffer, size);
	  delete(buffer);
	  if(retVal != size){
		  kernel->machine->WriteRegister(2, -1);
		  break;
