...
ExceptionHandler(ExceptionType which)
{
  ...
  switch (which) {
    case SyscallException:
	  ...
	  int chr;
          int name_addr = kernel->machine->ReadRegister(4);
          char *progName = new char[100];
	  int i = 0;
          while (i < 99 && chr!= 0) {
		  kernel->machine->ReadMem(name_addr+i, 1, &chr);
		  progName[i++] = (char) chr;
              }
          Thread* t = new Thread(progName);
          kernel->machine->WriteRegister(2, ++count);
          threadlist->Append(t);
	  t->Fork((VoidFunctionPtr) RunProg, (void*) progName);
	  //kernel->machine->WriteRegister(2, ++count);
	  //threadlist->Append(t);
	  }
	  break;
        ...
        case SC_Join:
	  {
          ...
          int thread_to_join_SpaceId = kernel->machine->ReadRegister(4); 
	  /*
	  ListIterator<Thread*> it(threadlist);
	  Thread* thread_to_join = it.Item();
	  Thread* currentThread = kernel->currentThread;
	  thread_to_join->waitinglist->Append(currentThread);
	  IntStatus oldLevel = kernel->interrupt->SetLevel(IntOff);
          currentThread->Sleep(FALSE);
          (void) kernel->interrupt->SetLevel(oldLevel);
          thread_to_join->Finish();
